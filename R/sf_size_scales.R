...
sf_set_text_sizes <- function(size = "xSmall") {
    ...
    ret <- theme()
    ret <-
        ret + theme(axis.text.x = element_text(size = axis_text_size))
    ret <-
        ret + theme(axis.text.y = element_text(size = axis_text_size))
    ret <-
        ret + theme(axis.title = element_text(size = axis_title_size))

    ret <-
        ret + theme(axis.title.x = element_text(size = axis_title_size))
    ret <-
        ret + theme(axis.title.y = element_text(size = axis_title_size))
    ret <- ret +
        theme(axis.title.y.right = element_text(size = axis_title_size))
    ret <-
        ret + theme(strip.text = element_text(size = strip_text_size))
    ret <-
        ret + theme(plot.title = element_text(size = plot_title_size))
    ret <-
        ret + theme(plot.subtitle = element_text(size = subtitle_size))
    ret <-
        ret + theme(plot.caption = element_text(size = caption_size))
